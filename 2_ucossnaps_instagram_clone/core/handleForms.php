...
if (isset($_POST['createAlbumBtn'])) {
    ...
    if (!empty($album_name)) {
        $result = insertAlbum($pdo, $album_name, $user_id);
        if ($result) {
            header("Location: ../index.php");
        }
    }
...
if (isset($_POST['updateAlbumBtn'])) {
    ...
    if (!empty($album_name)) {
        $stmt = $pdo->prepare("UPDATE albums SET name = :name WHERE album_id = :album_id");
        $stmt->execute([':name' => $album_name, ':album_id' => $album_id]);
        header("Location: ../index.php");
    } else {
