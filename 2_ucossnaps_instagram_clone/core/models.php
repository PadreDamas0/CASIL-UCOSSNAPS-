...
function insertAlbum($pdo, $name, $user_id) {
    $sql = "INSERT INTO albums (name, user_id) VALUES (?, ?)";
    $stmt = $pdo->prepare($sql);
    $executeQuery = $stmt->execute([$name, $user_id]);
    return $executeQuery;
}


function updateAlbumName($pdo, $album_id, $name) {
    $sql = "UPDATE albums SET name = ? WHERE album_id = ?";
    $stmt = $pdo->prepare($sql);
    $executeQuery = $stmt->execute([$name, $album_id]);
    return $executeQuery;
}
...
function associatePhotoWithAlbum($pdo, $album_id, $photo_id) {
